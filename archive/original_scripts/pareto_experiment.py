...
for name in names:
    ...
    feat_codes.append(pd.factorize(vals, sort=False)[0].astype(np.uint64)+1)
a_codes=pd.factorize([canon(x) for x in a],sort=False)[0].astype(np.uint64)+1
target_codes=pd.factorize([canon(tuple(int(x) for x in M.ravel())) for M in Wns],sort=False)[0].astype(np.int64)
...
# Save
res.to_csv('/mnt/data/pareto_all_511.csv',index=False)
fd.to_csv('/mnt/data/pareto_frontier_D_state.csv',index=False)
fa.to_csv('/mnt/data/pareto_frontier_A_freq.csv',index=False)
summary={
...
with open('/mnt/data/pareto_summary.json','w') as f: json.dump(summary,f,ensure_ascii=False,indent=2)
